  ...
  for (let i = 0; i < relays.length; i += 1) {
    ...
    if (relays[i].https) {
      ...
      if (!fs.existsSync(path.join(__dirname, `../build/certificates/${relays[i].serverName}`))) {
        ...
        await execProcess(`openssl req -help -x509 -nodes -days 365 -newkey rsa:2048 -keyout ${certificatePath}/privkey.pem -out ${certificatePath}/fullchain.pem -subj "/C=${serverConfig.address.country}/ST=${serverConfig.address.city}/L=${serverConfig.address.neighborhood}/O=${serverConfig.project} /OU=IT Department/CN=${relays[i].serverName}"`)
